...
-------------------------------------------------------------------------*/
// .NAME RowQuery - abstract interface for queries that return
...
// implement the RowQuery API to return query results:
//
...
class RowQuery : public Object
{
public:
  vtkTypeMacro(RowQuery, vtkObject);
  ...
  virtual std::string GetFieldName(int i) = 0;
...
private:
  RowQuery(const RowQuery &); // Not implemented.
  void operator=(const RowQuery &); // Not implemented.
};
