...
void RowQuery::PrintSelf(ostream &os, vtkIndent indent)
{
...
int RowQuery::GetFieldIndex(char* name)
{
  ...
  bool found = false;
  for (int index = 0; index < this->GetNumberOfFields(); index++)
    {
    if (this->CaseSensitiveFieldNames)
      {
      if (name.compare(this->GetFieldName(index)) == 0)
        {
        found = true;
        break;
        }
...
bool RowQuery::NextRow(vtkVariantArray* rowArray)
{
  if (!this->NextRow())
    {
    return false;
